...
exports.searchNews = async (req, res) => {
  try {
    ...
    const items = await News.find({
      ...
      .limit(limit)
      .select("title slug featuredImage category publishedDate createdAt readTime destination")
      .lean();
...
exports.getHomeFeed = async (req, res) => {
  try {
    ...
    const lite =
      // createdAt is explicitly listed: articles imported before the
      // publishedDate field existed would otherwise carry no date at all.
      "title slug subtitle description excerpt featuredImage category tags author publishedDate createdAt updatedAt readTime views likes shareCount destination region breakingNews featured trending editorsPick priority";
    ...
    const [hero, breaking, featured, editorsPick, trending, popular, latest] =
      await Promise.all([
        ...
        pick({}, 12, { views: -1 }),
        // The pool has to cover every rail at once — hero, breaking, featured,
        // editor's picks, trending, latest, don't miss and more-stories — with
        // no repeats, so it is sized well above their combined length.
        pick({}, 70),
      ]);

    /**
     * Rails driven by editorial flags are empty on a fresh install. Rather than
     * shipping blank homepage sections, unused recent articles are promoted —
     * each one used at most once so the page never repeats itself.
     */
    const used = new Set();
    const mark = (list) => {
    ...
    if (heroDoc) used.add(String(heroDoc._id));
  ...
  try {
    ...
    const byTag = current.tags?.length
      ? await News.find({ ...base, tags: { $in: current.tags } })
          ...
          .sort({ publishedDate: -1 })
          .limit(limit)
          .select("title slug featuredImage category publishedDate createdAt readTime excerpt description")
          .lean()
      : [];
    ...
    // A thin category (or an untagged article) would otherwise return nothing;
    // recent stories keep the "related" rail useful instead of empty.
    if (data.length < limit) {
      const usedIds = [current._id, ...data.map((n) => n._id)];
      const filler = await News.find({ ...base, _id: { $nin: usedIds } })
        .populate("category")
