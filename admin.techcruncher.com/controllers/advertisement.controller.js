...
const buildAdPayload = (body = {}) => {
  ...
  if (body.display !== undefined) patch.display = body.display === "frame" ? "frame" : "banner";

  if (body.maxHeight !== undefined) {
    const height = Number(body.maxHeight);
    // Clamped so a typo cannot book a banner taller than the viewport.
    patch.maxHeight =
      Number.isFinite(height) && height > 0 ? Math.min(Math.round(height), 1200) : null;
  }
