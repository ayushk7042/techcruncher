...
exports.getCategories = async (req, res) => {
  try {
    ...
    /**
     * ?withCover=true attaches the newest published article's image as
     * `coverImage`. Category art is optional in the admin, so without this the
     * public "browse by category" cards would have nothing to show — and one
     * aggregate is far cheaper than the frontend fetching an article per row.
     */
    if (req.query.withCover === "true") {
      const covers = await News.aggregate([
        {
          $match: {
            deletedAt: null,
            status: "published",
            "featuredImage.url": { $nin: [null, ""] },
          },
        },
        { $sort: { publishedDate: -1, createdAt: -1 } },
        { $group: { _id: "$category", image: { $first: "$featuredImage" } } },
      ]);

      const coverMap = new Map(covers.map((c) => [String(c._id), c.image]));
      categories = categories.map((c) => ({
        ...c,
        coverImage: c.image?.url ? c.image : coverMap.get(String(c._id)) || null,
      }));
    }
...
exports.reorderCategories = async (req, res) => {
  try {
    // The admin panel posts `items`; older clients post `order`. Accept both.
    const body = req.body || {};
    ...
    if (!items.length) {
      return res
        .status(400)
        .json({ success: false, message: "items array required: [{ id, order, parent? }]" });
    }
    ...
    await Promise.all(
      items
        ...
        .map((i) => {
          const patch = { order: Number(i.order) || 0 };
          // Reordering may also move a category under a different parent.
          if (i.parent !== undefined) patch.parent = isObjectId(i.parent) ? i.parent : null;
          return Category.findByIdAndUpdate(i.id, patch);
        })
    );
...
exports.toggleVisibility = async (req, res) => {
  try {
    // A bodyless PATCH is the common case ("just flip it"), so req.body may be
    // undefined — guard before reading, and fall back to inverting `hidden`.
    const body = req.body || {};
    const patch = {};

    if (body.hidden !== undefined) patch.hidden = toBool(body.hidden);
    if (body.showOnHome !== undefined) patch.showOnHome = toBool(body.showOnHome);
    if (body.showInMenu !== undefined) patch.showInMenu = toBool(body.showInMenu);
    if (body.showInFooter !== undefined) patch.showInFooter = toBool(body.showInFooter);
    ...
    if (!Object.keys(patch).length) {
      const current = await Category.findById(req.params.id).select("hidden").lean();
